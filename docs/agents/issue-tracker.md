...
- Linear team: CAG / Pi Symphony Runner
...
- `Ready for Agent` means a ticket is fully specified and safe for Pi Symphony to claim.
- `In Progress` means Pi Symphony has claimed the issue or a human is actively repairing it.
...
Create or update a Linear issue in the Pi Symphony Runner project. Use the Linear API with `LINEAR_API_KEY` from `.env.local`; never commit credentials.
...
GitHub is the code review and CI surface. Pi Symphony opens PRs from `symphony/<issue>-workspace` branches into the configured base branch and posts deterministic handoff comments.
