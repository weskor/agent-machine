...
- Before commit, push, PR create/update, or PR validation side effects, the runner writes `pr_handoff_pending` progress and a bounded PR handoff payload with issue identity, workspace, expected branch, and the advisory Agent PR URL. Inline PR handoff execution re-reads that persisted payload before side effects.
