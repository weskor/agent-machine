...
4. **Configurable runtime provider evidence** — persist selected provider/model evidence for supported local providers (`pi_cli` default, `codex_cli`) and future fake/API/app-server/ACP-style Adapters.
...
- Before commit, push, PR create/update, or PR validation side effects, the runner writes `pr_handoff_pending` progress and a bounded PR handoff payload with issue identity, workspace, expected branch, advisory Agent PR URL, and non-secret continuation facts for review/final handoff. Inline PR handoff execution re-reads that persisted payload before side effects. The selected `handoff` worker may claim a pending PR handoff payload through the run lease, execute PR handoff from that payload, then queue `review_pending` when review is configured or final `handoff_pending` when it is not.
...
- Before semantic review side effects, the runner writes `review_pending` progress and a bounded review payload with the PR URL, issue identity, workspace, branch, scope result, validation evidence, usage, and GitHub auth evidence. Inline review execution re-reads that persisted payload before collecting review evidence or invoking the review command. Review resume after `review_not_ready` uses the same payload execution boundary. The selected `review` worker may claim a pending review payload through the run lease, execute review from that payload, and write `handoff_pending` output for the handoff worker when review does not end terminally.
