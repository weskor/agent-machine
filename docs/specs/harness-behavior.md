...
- The PR/MR author invariant derives accepted GitHub App author forms from the configured app slug (`app/<slug>` and `<slug>[bot]`) or an explicit project config PR author override. GitLab MR ownership uses `gitlab.pr_author_override` or `GITLAB_PR_AUTHOR_OVERRIDE`. If no expected author source is available, merge automation fails closed with a clear ownership blocker.
