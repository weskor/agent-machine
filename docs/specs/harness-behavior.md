...
10. **Evidence/debug artifact schema validation** — define and validate schemas for run records, evaluation artifacts, deterministic handoff evidence, feedback files, and capped raw debug artifact metadata.
...
| Success with PR handoff | Valid PR exists, validation passed, scope guard passed or warned, and configured review is absent or passed. | Create/update the runner-owned PR body with handoff evidence, post/update Linear handoff comments, move the issue to Human Review, and write status `success`. | Runner-owned validation, PR identity, review status, PR body evidence, Linear comments, Linear transition, and run/evaluation artifacts. | SQLite records attempt success, PR mapping, handoff state, review classification, and merge blockers before artifacts/comments act as exports. |
