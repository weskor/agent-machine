...
3. **SQLite owns Pi Symphony decisions.** Candidate claims, local attempt status, retry/no-retry decisions, processed feedback hashes, review classifications, merge blockers, cleanup decisions, leases, heartbeats, and terminal outcomes come from current-state SQLite rows. For Done workspace cleanup, the durable issue-attempt run status is the cleanup-policy status when present; terminal/evaluation outcome rows remain diagnostic context and do not override a present run status.
