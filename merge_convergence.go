...
func handleAlreadyMergedMissingMergeTaskPR(ctx context.Context, client linearClient, config runnerConfig, store *orchstate.Store, github githubAPI, task orchstate.WorkerTask, payload mergeWorkerTaskPayload) (bool, string, error) {
	...
	facts, merged, err := mergedPullRequestFacts(ctx, github, prURL)
	if err != nil {
	...
	if issueKey == "" {
		return true, terminalPRReasonAlreadyMerged, fmt.Errorf("queued merge task %s has merged PR %s but no issue key", task.TaskKey, prURL)
	}
	candidate, err := client.issueByIdentifierContext(ctx, issueKey)
	if err != nil {
		return true, terminalPRReasonAlreadyMerged, err
	}
	if candidate == nil {
		return true, terminalPRReasonAlreadyMerged, fmt.Errorf("issue %s not found while converging already-merged PR %s", issueKey, prURL)
	}
	...
	return true, terminalPRReasonAlreadyMerged, nil
}
